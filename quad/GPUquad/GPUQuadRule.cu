...
namespace quad{
  ...
  template <typename T>
	...
	.25150011495314791996/(1<<NDIM), -.062875028738286979989/(1<<NDIM), -(-.062875028738286979989/(1<<NDIM)), .25150011495314791996/(1<<NDIM), -(.25150011495314791996/(1<<NDIM))
		...
		T cpuRule9G[] = {.47795365790226950619, .20302858736911986780, .44762735462617812882, .125, .34303789878087814570 };
  ...
  public:
	...
	void loadDeviceConstantMemory(int device = 0){
      ...
      QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermVarCount, cpuGenPermVarCount, sizeof(int)*fEvalPerRegion));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermGIndex, cpuGenPermGIndex, sizeof(int) *fEvalPerRegion));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermVarStart, cpuGenPermVarStart, sizeof(int) * (fEvalPerRegion+1)));
	  QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermVarStart, cpuGenPermVarStart, sizeof(int) * (fEvalPerRegion+1)));
	  //========================
	  //added by Ioannis
