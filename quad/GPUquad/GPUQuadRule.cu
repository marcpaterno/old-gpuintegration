...
namespace quad {
  ...
  class QuadRule {
    ...
    Rule9Generate()
    {
      T cRule9Wt[] = {
        NDIM * (NDIM *
                  (NDIM * (-.0023611709677855117884) + .11415390023857325268) +
                (-.63833920076702389094)) +
          .74849988504685208004,
        NDIM * (NDIM *
                  (NDIM * (-.0014324017033399125142) + .057471507864489725949) +
                (-.14225104571434243234)) -
          (-.062875028738286979989),
        NDIM * (.254591133248959089) -
          (NDIM * (NDIM * (NDIM * (-.0014324017033399125142) +
                           .057471507864489725949) +
                   (-.14225104571434243234)) -
           (-.062875028738286979989)),
        NDIM * (NDIM * (-1.207328566678236261) + .89567365764160676508) - 1 +
          NDIM * (NDIM * (NDIM * (-.0023611709677855117884) +
        ...
        .014689102496143490175 -
          (NDIM * (NDIM * .0021486025550098687713 + (-.032268563892953949998)) +
           .010636783990231217481),
        NDIM * .51134708346467591431 + .45976448120806344646 +
          NDIM * (NDIM * .0035417564516782676826 + (-.072609367395893679605)) +
          .10557491625218991012,
        ...
        -(.0035328099607090870236 - NDIM * .0021486025550098687713),
        .09231719987444221619 + .015532417276607053264 -
          NDIM * .0035417564516782676826,
        -(.015532417276607053264 - NDIM * .0035417564516782676826),
...
// changed by Ioannis
      ...
      // 1), 4*NDIM*(NDIM - 1), 4*NDIM*(NDIM - 1)*(NDIM - 2)/3, 1 << NDIM};
      ...
      T cpuRule9G[] = {.47795365790226950619,
  ...
  public:
    ...
    void
    loadDeviceConstantMemory(int device = 0)
    {

      Device.DeviceInit(device, VERBOSE);
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        gpuG, cpuG, sizeof(T) * NDIM * NSETS));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        cRuleWt, CPURuleWt, sizeof(T) * NRULES * NSETS));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        cGeneratorCount, CPUGeneratorCount, sizeof(size_t) * NSETS));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        GPUScale, CPUScale, sizeof(T) * NSETS * NRULES));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        GPUNorm, CPUNorm, sizeof(T) * NSETS * NRULES));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        gpuGenPos, genPtr, sizeof(int) * PERMUTATIONS_POS_ARRAY_SIZE));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        gpuGenPermVarCount, cpuGenPermVarCount, sizeof(int) * fEvalPerRegion));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(
        gpuGenPermGIndex, cpuGenPermGIndex, sizeof(int) * fEvalPerRegion));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermVarStart,
                                                      cpuGenPermVarStart,
                                                      sizeof(int) *
                                                        (fEvalPerRegion + 1)));
      QuadDebug(Device.CopyHostToDeviceConstantMemory(gpuGenPermVarStart,
    ...
    void
    Init(int ndim, size_t fEval, int key, int verbose)
    {
