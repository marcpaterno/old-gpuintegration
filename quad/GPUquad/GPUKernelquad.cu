...
namespace quad{
  ...
  template<typename T> 
	...
	public:
	  ...
	  template<typename integT>	  
	  int IntegrateSecondPhase(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, integT *test, T *optionalInfo = 0, Volume *vol = NULL){
...
#pragma omp parallel
		...
		BLOCK_INTEGRATE_GPU_PHASE2<T><<<numBlocks, numThreads, 0, stream[gpu_id]>>>(dRegionsThread, dRegionsLengthThread, numRegionsThread, dRegionsIntegral, dRegionsError, dRegionsNumRegion, activeRegions, subDividingDimension, epsrel, epsabs, fEvalPerRegion, NDIM, dTest, dvol);
