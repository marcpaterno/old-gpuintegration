...
kernel(cudaTextureObject_t texObj, cudaTextureObject_t texObj2)
{
  ...
  printf("highs:%f\n", tex2D<float>(texObj2, 0, 1));
  printf("highs:%f\n", tex2D<float>(texObj2, 0, 2));
}
...
namespace quad {
                         ...
                         int NDIM)
  {
    ...
    if (threadId < newNumOfRegions) {
      ...
      for (int dim = 0; dim < NDIM; ++dim) {
        ...
        // dim, newRegions[newNumOfRegions*dim + threadId], slength[dim]);
      }
    }
  }
  ...
  public:
    ...
    void
    InitGPUKernelCuhre(const int dim, int key, int verbose, int numDevices = 1)
    {
      QuadDebug(cudaDeviceReset());
      NDIM = dim;
      KEY = key;
    ...
    GenerateInitialRegions()
    {
      ...
      hRegions = (T*)Host.AllocateMemory(
        &hRegions, sizeof(T) * NDIM); // as many regions as dimensions for now
      hRegionsLength =
      ...
      size_t numBlocks =
        (size_t)ceil(pow((T)numOfDivisionPerRegionPerDimension, (T)NDIM) /
                     numThreads); // multiple subregions per block
      ...
      generateInitialRegions<T><<<numBlocks, numThreads, NDIM * sizeof(T)>>>(
        ...
        NDIM);
      QuadDebug(Device.ReleaseMemory(
        (void*)dRegions)); // with the new regions generated, we dont' need the
                           // memory of the original ones
    ...
    void
    GenerateActiveIntervals(int* activeRegions, int* subDividingDimension)
    {
      ...
      thrust::exclusive_scan(d_ptr,
                             d_ptr + numRegions,
                             scan_ptr); // creates running sum, with
                                        // scan_ptr[0]=0, for the scannedArray
      ...
      // std::cout << "GenerateActiveIntervals: 	numActiveRegions:" <<
      // numActiveRegions << "\n";
      ...
      if (numActiveRegions > 0) {
        ...
        /*if(numActiveRegions < (1 << 7)){
          ...
          numOfDivisionOnDimension = 1 <<
          ((int)ceil(log(numOfDivisionOnDimension)/log(2))-1); }else
          if(numActiveRegions < (1<<10)){ numOfDivisionOnDimension = 8; }else
          if(numActiveRegions < (1<<12)){ numOfDivisionOnDimension = 4; }else{
        ...
        if (VERBOSE) {
          sprintf(msg,
                  ...
                  "Regions\t\t: %ld\n\tDivision on dimension\t\t: %ld division",
                  numActiveRegions,
                  numOfDivisionOnDimension);
          Println(log, msg);
        }
                        ...
                        Volume* vol = NULL)
    {
      ...
      INTEGRATE_GPU_PHASE1<T><<<numBlocks, numThreads>>>(dRegions,
                                                         dRegionsLength,
                                                         numRegions,
                                                         dRegionsIntegral,
                                                         dRegionsError,
      ...
      // printf("After PHASE 1 Kernel 	nregions:%i
      // NumRegions:%i\n", nregions, numRegions); printf("integral:%f\n",
      // integral);
                        ...
                        Volume* vol = NULL)
    {
      ...
      for (int i = 0; i < 100; i++) {
        FirstPhaseIteration(
          epsrel, epsabs, integral, error, nregions, neval, dTest, dvol);
        if (VERBOSE) {
          sprintf(
            msg,
            "Iterations %d:\t%ld integrand evaluations so far\n%lf +- %lf ",
            i + 1,
            neval,
            integral,
            error);
...
#pragma omp parallel
      {
        ...
        if (cpu_thread_id < num_cpu_threads) {
          ...
          // std::cout << " phase2  # of BLOCKS:" << numBlocks << " # of
