...
namespace quad {
  ...
  public:
    ...
    void
    Allocate(float* arr, int rows, int cols)
    {
    ...
    void
    Initialize(float* xs, float* ys, float* zs, int rows, int cols)
    {
      // rows == 22
      _rows = rows;
      _cols = cols;
    ...
    void
    Initialize(Interp2D* orig, float* zs)
    {
    ...
    __device__ int
    SmallNearestN(cudaTextureObject_t array, float target, int size)
    {
      ...
      // find smallest neighbor, by finding first larger than target, then move
      // one spot back
    ...
    __device__ float Interp(double y, double x /*, cudaTextureObject_t interpT, cudaTextureObject_t interpC, cudaTextureObject_t interpR, int offset*/)
    {
      ...
      float y1, y2, x1, x2; // interpolation points, y-dimension is row
                            // dimension
      int y1_index = SmallNearestN(
        interpR, y, _rows); // get the index of nearest smaller neighbor
      ...
      // y, x1_index, y1_index, x1, x2, y1, y2);
      ...
      q12 =
        tex2D<float>(interpT, y1_index + 1, x1_index + (offset * INTERP_COLS));
      q21 =
        tex2D<float>(interpT, y1_index, x1_index + 1 + (offset * INTERP_COLS));
      q22 = tex2D<float>(
        interpT, y1_index + 1, x1_index + 1 + (offset * INTERP_COLS));

      double t1 = (x2 - x) / ((x2 - x1) * (y2 - y1));
      double t2 = (x - x1) / ((x2 - x1) * (y2 - y1));
      return ((q11 * (y2 - y) + q12 * (y - y1)) * t1 +
              (q21 * (y2 - y) + q22 * (y - y1)) * t2);
    }
