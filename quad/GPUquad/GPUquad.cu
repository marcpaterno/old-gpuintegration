...
#define TAG  0
    ...
    int integrate(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, integT *test1, Volume *vol = NULL)
	{
	  ...
	  if(numprocs > 1){
	    ...
	    kernel->IntegrateFirstPhase(epsrel, epsabs, integral, error, nregions, neval, test1);
		...
		timer::start_timer(&timer_one);
	      ...
	      errorFlag = kernel->IntegrateSecondPhase(epsrel, epsabs, integral, error, nregions, neval, test1 ,optionalInfo);
	    ...
	    //printf("Chronos time in milliseconds :%f\n",std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());
