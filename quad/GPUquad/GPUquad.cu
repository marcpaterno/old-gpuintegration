...
namespace quad{
  ...
  public:
    ...
    GPUcuhre(int pargc, char **pargv, int dim, int key = 0, int verbose = 0, int numDevices = 1){
      argc = pargc;
      argv = pargv;
...
#define TAG  0
    ...
    void MPI_CLIENT_checkCUDA(int nodeRank, T epsrel, T epsabs){
      MPI_Status stat;
	...
	if (devCount >= 1){
	...
	else
	  {
	    ...
	    sprintf(idstr, "%s|%5d|%4d|%s", processor_name, nodeRank, devCount, devProp.name);
	  }
      }
    ...
    void MPI_CLIENT_receiveQuadratureData(int nodeRank, int index, integT *test, T *numInfo){//numCUDADevices, unsigned long int numRegions){
	...
	endIndex = numRegions;
      ...
      if(VERBOSE){
	log << "\nNode " << nodeRank << "\nData start index : " << startIndex <<"\nData end index : " << endIndex << "\nNumber of regions per node : " << numRegionsPerNode << std::endl;
      }
	...
	errorFlag = thkernel->IntegrateSecondPhase(epsrel, epsabs, th_integral, th_error, th_nregions, th_neval, test, optionalInfo);
	...
	if(VERBOSE){
	  log << "\nIntegral:" << th_integral << "\nError:" << th_error << "\nRegions:" << th_nregions << "\nFunction Evaluations:" << th_neval  << "\nError:" << errorFlag << std::endl;
			...
			sprintf(msg, "Second Phase time at node %d : %.2lfms", nodeRank, time);
			Print(msg);
		#endif
	}	
      }
    ...
    std::map<int , std::vector<std::string> > MPI_MASTER_findActiveCUDANodes(int nodeRank, int numprocs, char *processor_name){
      MPI_Status stat;
      char buff[BUFSIZE];
	...
	if(buff[0] == '+'){
	  ...
	  if(nodeName.compare(processor_name)!=0 && nodes.find(nodeName) == nodes.end()){
	...
	log << "-----  ------------- ----- \n";
	for (std::map<int , std::vector<std::string> >::iterator it = CUDANodeDetails.begin(); it!= CUDANodeDetails.end(); ++it){
	  log << it->first << " | " << it->second[1].c_str() << " | " << it->second[3].c_str() << std::endl;
	}      
      }
    ...
    template<typename integT>
    int MPI_MASTER_integrateSecondPhase(GPUKernelCuhre<T> *thkernel, unsigned long int numCUDADevices, T &integral, T &error, size_t &nregions, size_t &neval, integT *test, T *optionalInfo){
      int devCount = 0;
      cudaGetDeviceCount(&devCount);
	...
	log << "\nNode " << nodeRank << "\nData start index : " << startIndex <<"\nData end index : " << endIndex << "\nNumber of regions per node : " << numRegionsPerNode << std::endl;
      ...
      timer::start_timer(&timer_node);
#endif
      int errorFlag = thkernel->IntegrateSecondPhase(epsrel, epsabs, integral, error, nregions, neval, test, optionalInfo);
    ...
    void MPI_MASTER_sendQuadratureData(GPUKernelCuhre<T> *thkernel, std::map<int, std::vector<std::string> > CUDANodeDetails, int numCUDADevices){
    ...
    template<typename integT>
    T MPI_MASTER_integrateFirstPhase(GPUKernelCuhre<T> *thkernel, T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, integT *test){

      if(VERBOSE){
			sprintf(msg, "Cuhre input parameters:\nndim %ld\nepsrel %e\nepsabs %e\nkey %ld\n\n",
					NDIM, epsrel, epsabs,KEY);
			Print(msg);
      }
      ...
      //thkernel->IntegrateFirstPhase(epsrel, epsabs, integral, error, nregions, neval, 0);
    ...
    template<typename integT>
    int MPI_INTEGRATE(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, integT *test)
	{
	  ...
	  if(id == 0)
		...
		log.open(logfilename.c_str());
	      ...
	      T firstPhaseTime = 0;
	      firstPhaseTime = MPI_MASTER_integrateFirstPhase(thkernel, epsrel, epsabs, integral, error, nregions, neval, test);

	      size_t numRegions =  thkernel->getNumActiveRegions();
	      std::map<int , std::vector<std::string> > CUDANodeDetails = MPI_MASTER_findActiveCUDANodes(id, numprocs, processor_name);
		...
		index += atoi(it->second[3].c_str());
	      ...
	      for(int node_id = 1; node_id < numprocs; ++node_id){
		MPI_Send(activeCUDADevicesIdx, numprocs, MPI_INT, node_id, TAG, MPI_COMM_WORLD);
	      }
	
	      //Send Quadrature data
	      MPI_MASTER_sendQuadratureData(thkernel, CUDANodeDetails, numCUDADevices);
	      ...
	      T *optionalInfo = (T *)malloc(sizeof(T) * 2);
	      if(thkernel->getNumActiveRegions() > 0){
		errorFlag = MPI_MASTER_integrateSecondPhase(thkernel, numCUDADevices, integral, error, nregions, neval, test, optionalInfo);
	      }
	      ...
	      for (std::map<int, std::vector<std::string> >::iterator it = CUDANodeDetails.begin(); it!= CUDANodeDetails.end(); ++it){
		T result[6];
		MPI_Recv(result, 6, MPI_DOUBLE, it->first, TAG, MPI_COMM_WORLD, &stat);
		integral += result[0];
		error += result[1];
...
#endif
	      printf("%d\t%e\t%.10lf\t%.10f\t%-15ld\t%-15ld\t%d\t%-10.2lf\t%-10.2lf\n", DIM, epsrel, integral, error, nregions, neval, errorFlag, kernelTime+firstPhaseTime, mpiCommTime+totalTime);
	  ...
	  else
	    {
	      ...
	      MPI_Recv(activeCUDADevicesIdx, numprocs, MPI_INT, 0, TAG, MPI_COMM_WORLD, &stat);
		...
		if(VERBOSE)
		  log.open(logfilename.c_str());
		MPI_CLIENT_receiveQuadratureData(id, activeCUDADevicesIdx[id],test, numInfo);
	      }
	      //MPI_Finalize();
	    }
	  return errorFlag;
    ...
    template<typename integT>
    int integrate(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, integT *test1, Volume *vol = NULL)
	{
	  ...
	  if(numprocs > 1){
	    errorFlag = MPI_INTEGRATE(epsrel, epsabs, integral, error, nregions, neval, test1);
	    MPI_Finalize(); 
	  }
	  else{
#if TIMING_DEBUG == 1
	    ...
	    kernel->IntegrateFirstPhase(epsrel, epsabs, integral, error, nregions, neval, test1, vol);
	    ...
	    if(kernel->getNumActiveRegions() > 0){
	      ...
	      errorFlag = kernel->IntegrateSecondPhase(epsrel, epsabs, integral, error, nregions, neval, test1 ,optionalInfo, vol);
	      

#if TIMING_DEBUG == 1		
	      secondPhaseTime = timer::stop_timer_returntime(&timer_one, "Second Phase");
#endif
