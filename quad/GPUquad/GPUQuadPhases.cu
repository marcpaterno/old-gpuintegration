...
namespace quad {
                       ...
                       Volume* vol = NULL)
  {
    ...
    int fail = 0;
    INIT_REGION_POOL(
      dRegions, dRegionsLength, numRegions, fEvalPerRegion, NDIM, test, vol);
  ...
  INSERT_GLOBAL_STORE(Region* sRegionPool, Region* gRegionPool)
  {
    ...
    // gPool[gRegionPoolSize + threadIdx.x] = sRegionPool[BLOCK_SIZE +
    // threadIdx.x];
                             ...
                             Volume* vol = NULL)
  {
    ...
    for (; (nregions <= MAX_GLOBALPOOL_SIZE) &&
           (nregions == 1 || ERR > MaxErr(RESULT, epsrel, epsabs));
         ++nregions) {
      gRegionPool = gPool;
      sRegionPoolSize =
        EXTRACT_MAX<T>(sRegionPool, gRegionPool, sRegionPoolSize);
      ...
      __syncthreads();
      SampleRegionBlock<T>(
        sRegionPoolSize - 1, fEvalPerRegion, NDIM, test, vol);
