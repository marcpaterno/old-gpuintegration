...
//#define TYPE double
__device__ float Interp(double y, double x, cudaTextureObject_t interpT, cudaTextureObject_t interpR, cudaTextureObject_t interpC, int offset);
__host__ float Interp(double y, double x, float *interpT, float *interpR, float *interpC);

typedef bool GPU;
typedef int CPU; 

template<typename T> __device__ T r8vec_sum ( int n, const T a[] )
{
  T sum;
  sum = 0.0;
  for (int i = 0; i < n; i++ ){
...
template<typename T> __device__ T r8vec_dot ( int n, T a1[], const T a2[] )
{
  int i;
  T value;
...
template<typename T> 
__device__ T IntegrandFunc(const T xx[], int NDIM) 						   
{
  T f = 0;
 ...
 /*  T tau 		=  	Interp(xx[0], xx[1], interpT, interpR, interpC, 0);
   T mu 		= 	Interp(xx[3], xx[2], interpT, interpR, interpC, 1);
   T sigma 		=	Interp(xx[7], xx[5], interpT, interpR, interpC, 2);
   T fmsk 		= 	Interp(xx[3], xx[6], interpT, interpR, interpC, 3);
   T fprj 		=	Interp(xx[0], xx[5], interpT, interpR, interpC, 4);
  */
  ...
  /*T highs[3] = {2, .3, 1};
	...
	// zt = .12;
	 ...
	 {
		 printf("tau:%f mu:%f sigma:%f fmsk:%f fprj:%f\n", tau, mu, sigma, fmsk, fprj); 
	 }
		
	 T exptau = exp(tau * (2.0 * mu + tau * sigma * sigma - 2.0 * lc) / 2.0);
     T root_two_sigma = sqrt(2.0) * sigma;
     T mu_tau_sig_sqr = mu + tau * sigma * sigma;
