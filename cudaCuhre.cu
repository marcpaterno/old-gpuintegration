...
int main(int argc, char **argv){
  ...
  //int errorFlag = cuhre->integrate(epsrel, epsabs, integral, error, nregions, neval, &t, &vol);
  ...
  integral = 0; error = 0;
   nregions = 0; neval = 0;
  errorFlag = cuhre.integrate(1e-7, epsabs, integral, error, nregions, neval, &t, &vol);
  //int errorFlag = cuhre->integrate(epsrel, epsabs, integral, error, nregions, neval, &t);

  printf("%d\t%e\t%.10lf\t%.10f\t%ld\t%ld\t%d\n", dim, 1e-7, integral, error, nregions, neval, errorFlag);
